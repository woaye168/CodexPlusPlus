...
//!
//! 本模块从 cc-switch 的对应实现移植并适配到 CodexPlusPlus 的存储与错误风格。
...
const TOKEN_REFRESH_BUFFER_MS: u64 = 60_000;
const DEVICE_CODE_DEFAULT_EXPIRES_IN: u64 = 900;
const POLLING_SAFETY_MARGIN_SECS: u64 = 3;
const CODEX_USER_AGENT: &str = "codex-plus-plus-codex-oauth";
...
impl CachedAccessToken {
    fn is_expiring_soon(&self) -> bool {
        let now = now_ms();
        self.expires_at_ms.saturating_sub(now) < TOKEN_REFRESH_BUFFER_MS
    }
}
...
impl From<&CodexAccountData> for CodexOAuthAccount {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CodexOAuthDeviceCodeResponse {
...
impl CodexOAuthManager {
    ...
    pub async fn start_device_flow(&self) -> Result<CodexOAuthDeviceCodeResponse, CodexOAuthError> {
        ...
        Ok(CodexOAuthDeviceCodeResponse {
    ...
    pub async fn poll_for_token(
        &self,
        device_code: &str,
    ) -> Result<Option<CodexOAuthAccount>, CodexOAuthError> {
        let entry = {
        ...
        if !status.is_success() {
            ...
            return Err(CodexOAuthError::TokenFetchFailed(format!("{status} - {text}")));
        }
    ...
    async fn add_account_internal(
        ...
        let mut account = CodexOAuthAccount::from(&data);
        ...
        self.save_to_disk().await?;
        account.is_default = self.resolve_default_account_id().await.as_deref() == Some(&account.id);
        Ok(account)
    }
    ...
    fn sorted_accounts(
        ...
        let mut list: Vec<CodexOAuthAccount> = accounts.values().map(CodexOAuthAccount::from).collect();
        list.sort_by(|a, b| {
        ...
        for account in &mut list {
            account.is_default = default_account_id == Some(account.id.as_str());
        }
        list
    }
...
fn compute_expires_at_ms(expires_in: Option<i64>) -> u64 {
    let secs = expires_in.unwrap_or(3600).max(1) as u64;
    now_ms() + secs * 1000
}
